...
public class ContextGenerator : IIncrementalGenerator
{
	...
	{
		var types = context.CompilationProvider.SelectMany(static (c, _) => c.SourceModule.ReferencedAssemblySymbols)
						   .Where(
								static a => a.Name == BenchmarkCore
										 || a.Modules.Any(
												m => m.ReferencedAssemblies.Any(ra => ra.Name == BenchmarkCore)))
						   .SelectMany(static (a, _) => GetAllNamespaces(a.GlobalNamespace))
						   .SelectMany(static (ns, _) => ns.GetTypeMembers())
						   .Combine(
								context.CompilationProvider.Select(
									static (c, _) => c.GetTypeByMetadataName(BenchmarkCoreIContext)))
						   .Where(
								static tuple =>
									tuple.Right != null
		...
		var enumerableType = context.CompilationProvider.Select(
			static (c, _) =>
			{
				...
				var type = enumerableType.Construct(
					c.CreateTupleTypeSymbol(
		...
		(ImmutableArray<string> Types, string? EnumerableType) parameters)
	{
		if (parameters.Types.Length   == 0
		 || parameters.EnumerableType == null)
			return;
		try
		{
		...
		string contextEnumerable)
	{
		...
		sb.Append("namespace Benchmark")
		  .Append(nl)
		  .Append("{")
		  .Append(nl);
		sb.Append("\tinternal static partial class Contexts")
		  .Append(nl)
		  .Append("\t{")
		  .Append(nl);
