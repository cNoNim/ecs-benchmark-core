	...
	where TIncrementalGenerator : IIncrementalGenerator, new()
{
	...
	public static DiagnosticResult CompilerInfo(string compilerInfo) =>
		new(compilerInfo, DiagnosticSeverity.Info);
		...
		params string[] expectedFileNames)
	{
		...
		var executingAssemblyPath = Path.GetDirectoryName(
		...
		if (checksGeneratedSource)
		{
			...
			var foundExpectedFiles = Directory.EnumerateFiles(generatedFolderPath)
											  .Select(Path.GetFileName)
											  .Where(expectedFileNames.Contains);
			var existingSources = foundExpectedFiles.Select(
				file =>
				{
					if (file == null)
	...
	private class Test : SourceGeneratorTest<DefaultVerifier>
	{
		...
		{
			get => Enum.TryParse("Default", out LanguageVersion version) ? version : LanguageVersion.CSharp9;
		}
		...
		private static string GetFilePathPrefixForGenerator(Type sourceGenType) =>
			Path.Combine(
				sourceGenType.Assembly.GetName()
							 .Name
			 ?? string.Empty,
				sourceGenType.FullName!);
